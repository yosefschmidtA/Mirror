...
nchanel=21 ; number of points in the spectrum. Should be changed for each region 

; variable definition
;for theta=thetai,thetaf,dtheta do begin
;j=0
;if theta le 60 then nchanel=33
...
Param=[[9.7D,1.9D,241571.0D],[12.6D,1.9D,87221.0D]]
...
;Shirley background
initback=1   ; you can change this number as you like. 
...
fitting='yes'  ; use only 'yes' or 'no'
...
if fitting eq'yes' then begin
	...
	;peak position constrain;
        ...
        ;constrains(0).limits(*)=[9.70D,9.70D] & constrains(0).limited(*)=[1,1]
        ;constrains(3).limits(*)=[13.0D,13.0D] & constrains(3).limited(*)=[1,1]
        ;constrains(6).limits(*)=[37.05D,38.0D] & constrains(6).limited(*)=[1,1]
        constrains(0).fixed=1 & Param(0)=9.7D
        constrains(3).fixed=1 & Param(3)=12.6D

        ;sigma (peak width);
        ;constrains(1).limits(*)=[3.1D,6.1D] & constrains(1).limited(*)=[1,1]
        ;constrains(4).limits(*)=[1.8D,2.1D] & constrains(4).limited(*)=[1,1]
        constrains(1).fixed=1 & Param(1)=1.9D
        constrains(4).fixed=1 & Param(4)=1.9D
        ;constrains(7).fixed=1 & Param(7)=1.85D
        ;constrains(1).limits(*)=[1.86D,1.86D] & constrains(1).limited(*)=[1,1]
        ;constrains(4).limits(*)=[1.82D,1.82D] & constrains(4).limited(*)=[1,1]
	...
	;area constrain
	constrains(2).limits(*)=[0.0D,2000000.0D] & constrains(2).limited(*)=[1,1]
	constrains(5).limits(*)=[0.0D,2000000.0D] & constrains(5).limited(*)=[1,1]
...
; or smoothing strongly for some angles
     ...
     auxiliar=smooth(auxiliar,smooth,/NAN)
...
;preparing mscdinput
kphoton=19.59  ; change it for the correct k value each time
thetaerro=0.2  ; correcting experimental polar angle constant.
; Open a new file to write
OPENW, 1, 'exjn1207_Pd3d.mscd'
...
printf,1,' '
printf,1,' provided by Pancotti et al. (LNLS in 12, June 2007) '
