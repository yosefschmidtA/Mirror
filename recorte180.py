...
# Função para filtrar dados de 0 a 180 graus
def filter_data_0_180(df):
    """
    Filtra os dados para incluir apenas valores de Phi entre 0 e 180 graus.
    """
    df_filtered = df[df['Phi'] <= 180].copy()
    return df_filtered
...
def save_to_txt_with_blocks(df, file_name):
    """
    Salva os dados organizados em blocos, limitados a Phi de 0 a 180 graus.
    """
...
# Filtrar os dados de 0 a 180 graus
df_filtered = filter_data_0_180(df)

# Salvar apenas os dados de 0 a 180 graus
output_file = 'exp_Fe2_GaO_filtered_0_180.txt'
save_to_txt_with_blocks(df_filtered, output_file)

print(f"Arquivo salvo com os dados de 0 a 180 graus: {output_file}")
